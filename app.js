...
// console.log(game.items.filter(item => item.name === "pokeball"))
game.catchPokemon(pokemon[142])
// console.log(game)
...
game.gymStatus = function(){
    ...
    completedGym = game.gyms.filter(gym => gym.completed === true)
    gymTally.completed = completedGym.length

    notCompletedGym = game.gyms.filter(gym => gym.completed === false)
    gymTally.incomplete = notCompletedGym.length
...
let hardGym = game.gyms.filter(gym => gym.difficulty < 8 )
...
// console.log(pokemon.find(poke => poke.name === "beans"))

game.catchPokemon = function (pokemonName) { 
  let pokeballs = game.items.find(item => item.name === "pokeball")
  pokemonName = pokemonName.toLowerCase()
  let pokemonObj = pokemon.find(mon => mon.name.toLowerCase() === pokemonName) 
  if (!pokemonObj){
